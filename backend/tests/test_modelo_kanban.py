...
def criar_quadro_lista_cartao(cliente: TestClient) -> tuple[dict, dict, dict]:
    ...
    lista = cliente.post(
        f"/quadros/{quadro['id']}/listas", json={"nome": "A fazer", "posicao": 1.0}
    ).json()
    cartao = cliente.post(
        f"/quadros/{quadro['id']}/listas/{lista['id']}/cartoes",
        json={"titulo": "Lavar roupa", "posicao": 1.0},
    ).json()
...
def test_mover_cartao_entre_listas_altera_apenas_lista_id(cliente: TestClient):
    ...
    lista_destino = cliente.post(
        f"/quadros/{quadro['id']}/listas", json={"nome": "Pronto", "posicao": 2.0}
    ).json()

    resposta = cliente.post(
        f"/quadros/{quadro['id']}/listas/{lista_origem['id']}/cartoes/{cartao['id']}/mover",
        json={"lista_id": lista_destino["id"], "nova_posicao": 1.0},
    )
    assert resposta.status_code == 200
    cartao_movido = resposta.json()

    # Mudou o que deveria mudar (a lista, e a posição pedida)...
    assert cartao_movido["lista_id"] == lista_destino["id"]
    assert cartao_movido["posicao"] == 1.0
...
def test_listas_de_um_quadro_voltam_na_ordem_definida_por_posicao(cliente: TestClient):
    ...
    # Criadas fora de ordem de propósito: se a API devolvesse na ordem de
    # criação (ou de id), este teste pegaria isso.
    cliente.post(f"/quadros/{quadro['id']}/listas", json={"nome": "Pronto", "posicao": 3.0})
    cliente.post(f"/quadros/{quadro['id']}/listas", json={"nome": "A fazer", "posicao": 1.0})
    cliente.post(f"/quadros/{quadro['id']}/listas", json={"nome": "Fazendo", "posicao": 2.0})
...
def test_arquivar_lista_arquiva_seus_cartoes(cliente: TestClient):
    ...
    segundo_cartao = cliente.post(
        f"/quadros/{quadro['id']}/listas/{lista['id']}/cartoes",
        json={"titulo": "Passar roupa", "posicao": 2.0},
    ).json()
...
def test_cartao_sem_prazo_e_valido(cliente: TestClient):
    ...
    resposta = cliente.post(
        f"/quadros/{quadro['id']}/listas/{lista['id']}/cartoes",
        json={"titulo": "Cartão sem data nenhuma", "posicao": 1.0},
    )
